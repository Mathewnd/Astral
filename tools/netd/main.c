	...
	while (i < optsize) {
		switch (opts[i]) {
			...
			case OPTION_LEASETIME:
				...
				break;
			case OPTION_DNS:
				uint32_t dns = optget32(opts, i + 2);
				offer->dns = dns;
				break;
			case OPTION_END:
				goto leave;
...
static void writedns(uint32_t dns) {
	char buff[100];
	int fd = creat("/etc/resolv.conf", 0644);
	if (fd < 0) {
		perror("failed to create /etc/resolv.conf");
		return;
	}

	struct in_addr addr;
	addr.s_addr = htonl(dns);
	char *ipstr = inet_ntoa(addr);
	
	snprintf(buff, 100, "# generated by netd\nnameserver %s\n", ipstr);
