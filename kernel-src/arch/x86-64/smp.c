...
static void cpuwakeup(struct limine_smp_info *info) {
	...
	__atomic_add_fetch(&cpusawake, 1, __ATOMIC_SEQ_CST);
	// TODO let other processors run threads and applications
	for (;;) CPU_HALT();
}
...
void arch_smp_wakeup() {
	...
	if (wakeupfn == cpuwakeup)
		while (__atomic_load_n(&cpusawake, __ATOMIC_SEQ_CST) != response->cpu_count) asm("pause");
