...
void arch_extracontext_save(extracontext_t *context) {
	...
	asm volatile("mov %%gs, %0" : "=r"(context->gs));
	asm volatile("mov %%fs, %0" : "=r"(context->fs));
...
void arch_extracontext_load(extracontext_t *context) {
	if (current_thread()->proc && current_thread()->proc->ldt)
		...
		arch_gdt_set_ldt(NULL, 0);

	void *old_base = current_cpu();
	if (!load_seg_gs(context->gs)) load_seg_gs(0);
	wrmsr(MSR_GSBASE, (uint64_t)old_base);
	wrmsr(MSR_KERNELGSBASE, context->gsbase);

	if (!load_seg_fs(context->fs)) load_seg_fs(0);	
