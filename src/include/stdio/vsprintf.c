...
static char* __int_str(uintmax_t i, char b[], int base, bool plusSignIfNeeded, bool spaceSignIfNeeded,
...
static void displayCharacter(char c, int* a, char* s) {
	s[*a] = c;
	*a += 1;
}
 
static void displayString(char* c, int* a, char* s) {
    for (int i = 0; c[i]; ++i) {
        displayCharacter(c[i], a, s);
    }
}
    ...
    for (int i = 0; format[i]; ++i) {
        ...
        if (format[i] == '%') {
                ...
                case 'u':
                {
                    switch (length) {
                        case 0:
                        {
                            ...
                            displayString(intStrBuffer, &chars, s);
                            break;
                        }
                        case 'H':
                        {
			...
			__int_str(integer, intStrBuffer, base, plusSign, spaceNoSign, lengthSpec, leftJustify, zeroPad);
                ...
                {
                    if (length == 'l') {
                        displayCharacter(va_arg(list, wint_t), &chars, s);
                    } else {
                        displayCharacter(va_arg(list, int), &chars, s);
                    }
                ...
                case 's':
                {
                    displayString(va_arg(list, char*), &chars, s);
                    break;
                }
            ...
            displayCharacter(format[i], &chars, s);
        }
    }
